...
test('Verify single active execution rule', async ({ page }) => {
    ...
    await expect(page.getByRole('row', { name: /61Execution/ }).getByTestId('buttonRoot')).toBeDisabled();
    await expect(page.locator('tbody button[data-testid="buttonRoot"]:disabled')).toHaveCount(1);
